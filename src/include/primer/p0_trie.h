...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    // Check if the last character already exists
    if (test_node->HasChild(key[length - 1])) {
      // If the last character is a terminal node, insertion fails
  ...
  bool Remove(const std::string &key) {
    ...
    // 检查最后一个节点是否为终止节点，如果不是终止节点，意味着该键不存在于Trie中
    if (!test_node->IsEndNode()) {
      latch_.WUnlock();
      return false;
    }
    // Set the end node to fasle
    test_node->SetEndNode(false);
    // 删除所有的孤立的节点,这个情况对应的是当前的节点是根节点，且完全找到了key的情况下才会这样的
